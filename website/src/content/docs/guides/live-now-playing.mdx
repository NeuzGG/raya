...
<Aside type="tip">The [official Raya bot](../../getting-started/official-bot/) has this built in: set `LIVE_FEED_PORT=8787` in `bot/.env` and skip to step 2.</Aside>
...
4. **Point the website at it.**
   - **GitHub Pages:** in your repository go to **Settings → Secrets and variables → Actions → Variables**, add `LIVE_FEED_URL` = `https://your-address/now-playing`, then re-run the **Deploy docs** workflow.
   - **Anywhere else:** set `liveFeed` in `website/src/site.mjs`, or the `PUBLIC_LIVE_FEED_URL` environment variable when building.

</Steps>

The landing page then shows a **Now playing live** card, and the floating card in the hero shows the bot's real song. Without a feed, both stay hidden, so the site never shows fake data.
